...
Status: artifacts committed; the two dashboard steps below are the human part.
...
## Step 1 — Supabase Storage bucket (CVs)

From `backend/` (reads `SUPABASE_URL` + `SUPABASE_SERVICE_KEY` from `.env`;
...
Expected: `bucket 'cvs': created, PRIVATE` (idempotent on re-run; refuses
to bless a bucket that is public).
...
2. Fill the prompted secrets (`sync: false` — asked once, stored by Render):

| Prompt | Value (source) |
|---|---|
| `DATABASE_URL` (api + cron) | Supabase **session pooler** URL from `backend/.env` (`...pooler.supabase.com:5432/postgres`) |
| `GLM_API_KEY` (api + cron) | Z.ai key from `backend/.env` |
| `CORS_ORIGINS` (api) | `https://jobfinderos.pages.dev` (Step 3's URL; add custom domains later, comma-separated) |
| `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` (api) | from `backend/.env` |
| `JOBTECH_API_KEY` / `REED_API_KEY` (api + cron) | from `backend/.env` (empty prompt = source self-disables) |
| `SENTRY_DSN` (api + cron) | empty = off |
